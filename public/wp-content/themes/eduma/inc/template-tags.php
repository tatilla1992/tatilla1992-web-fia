		...
		<?php if ( ! empty( $prev_post ) || ! empty( $next_post ) ): ?>
			<div class="entry-navigation-post">
				...
				if ( ! empty( $prev_post ) ):
					...
					<div class="prev-post">
						<p class="heading"><a
								href="<?php echo get_permalink( $prev_post->ID ); ?>"><?php echo esc_html__( 'Previous post', 'eduma' ); ?></a>
						</p>
						<h5 class="title">
				...
				if ( ! empty( $next_post ) ):
					...
					<div class="next-post">
						<p class="heading"><a
								href="<?php echo esc_url( get_permalink( $next_post->ID ) ); ?>"><?php echo esc_html__( 'Next post', 'eduma' ); ?></a>
						</p>
						<h5 class="title">
...
if ( ! function_exists( 'thim_entry_meta' ) ) :
	...
	function thim_entry_meta() {
		if ( is_front_page() || is_home() ) {
			$template       = 'thim_front_page_template';
			$display_layout = 'thim_front_page_cate_display_layout';
		} else {
			$display_layout = 'thim_archive_cate_display_layout';
			$template       = 'thim_archive_cate_template';
		}
		?>
		...
		<ul class="entry-meta">
			...
			if ( get_theme_mod( 'thim_show_category', false ) && get_the_category() ) {
				?>
			...
			if ( get_theme_mod( 'thim_show_date', true ) && ( get_theme_mod( $template, 'default' ) != 'default' || is_single() || get_theme_mod( $display_layout, false ) ) ) {
				?>
				<li class="entry-date">
			...
			if ( get_theme_mod( 'thim_show_tag', false ) ) {
				?>
				<?php if ( ! post_password_required() && ( comments_open() || '0' != get_comments_number() ) ) :
					?>
					<li class="entry-category">
