...
if ( ! function_exists( 'thim_get_feature_image' ) ) {
	function thim_get_feature_image( $attachment_id, $size_type = null, $width = null, $height = null, $alt = null, $title = null, $no_lazyload = null ) {
		...
		if ( $width && $height ) {
			$image_size = array( trim( $width ), trim( $height ) );
		} else {
			$image_size = ! $size_type ? $size_type : 'full';
		}
		...
		if ( $src ) {
			...
			$src_image = apply_filters( 'thim_image_default', THIM_URI . 'images/demo_images/demo_image.jpg' );
		}

		return sprintf( '<img src="%s" alt="%s" %s>', esc_url( $src_image ), esc_attr( $alt ), $style );
	}
}
...
if ( ! function_exists( 'thim_body_classes' ) ) {
	function thim_body_classes( $classes ) {
		...
		$classes[] = 'nofixcss';
		if ( get_theme_mod( 'navbar_mobile_show', false ) && ! empty( get_theme_mod( 'nav_mobile_item', [ 'home', 'course', 'search', 'account' ] ) ) ) {
			$classes[] = 'has-navbar-mobile';
		}
 		return $classes;
	}
}
 ...
 * Check is shop
 */
if ( ! function_exists( 'thim_check_is_shop' ) ) {
	function thim_check_is_shop() {
...
if ( ! function_exists( 'thim_check_courses_page' ) ) {
	function thim_check_courses_page() {
		if ( function_exists( 'learn_press_get_page_id' ) && ( is_post_type_archive( 'lp_course' ) || is_page( learn_press_get_page_id( 'courses' ) ) ) ) {
			return true;
		} else {
			return false;
		}
...
if ( ! function_exists( 'thim_print_preload' ) ) {
	function thim_print_preload() {
		$enable_preload     = get_theme_mod( 'thim_preload', '' );
...
if ( ! function_exists( 'thim_print_copyright' ) ) {
	function thim_print_copyright() {
		$copyright_text      = get_theme_mod( 'thim_copyright_text', 'Education WordPress Theme by ThimPress' );
		$show_copyright      = get_theme_mod( 'thim_copyright_show', true );
		$is_active_copyright = is_active_sidebar( 'copyright' );
		if ( $show_copyright && ( $copyright_text || $is_active_copyright ) ) { ?>
					...
					<div class="copyright-content">
						<div class="row">
							...
							if ( $is_active_copyright ) {
								echo '<div class="col-sm-' . ( 12 - get_theme_mod( 'thim_copyright_column', 6 ) ) . ' text-right">';
...
if ( ! function_exists( 'thim_upload_language_files' ) ) {
	function thim_upload_language_files() {
		...
		$curent_lange = get_locale();
		theme_eduma_download_file_language($curent_lange);
	}
}
...
add_action( 'update_option_WPLANG', 'thim_detect_language_change', 10, 2 );
function theme_eduma_download_file_language($curent_lang = null) {
	...
	$required         = false;
  	foreach ( $default_lang as $k => $val ) {
		...
		if ( isset($curent_lang ) && ($curent_lang  != str_replace( $prefix . '-', '', $k )) ) {
			continue;
		}
...
function thim_detect_language_change( $old_value, $new_value ) {
 	if ( $old_value !== $new_value ) {
		// check and download file language when change language
		theme_eduma_download_file_language($new_value);
 	}
}
...
if ( ! function_exists( "thim_function_event_booking" ) ) {
	function thim_function_event_booking () {
		$theme_options_data = get_theme_mods();
	...
	add_action( 'thim_event_booking', 'thim_function_event_booking' );
}

if ( ! function_exists( 'thim_event_nav' ) ) {
	function thim_event_nav() {
		...
		$key = array_search( get_the_ID(), $query );
		...
		<div class="entry-navigation-post">
            ...
            <div class="prev-post">
                <div class="content">
                    <p class="heading"><i class="tk tk-arrow-left"></i><?php echo esc_html__( 'Previous Event', 'eduma' ); ?></p>
                    <h5 class="title">
            ...
            <div class="next-post">
                <div class="content">
                    <p class="heading"><?php echo esc_html__( 'Next Event', 'eduma' ); ?><i class="tk tk-arrow-right"></i></p>
                    <h5 class="title">
	...
	add_action( 'tp_event_after_single_event', 'thim_event_nav', );
}

if ( ! function_exists( 'thim_ajax_load_content_post' ) ) {
	function thim_ajax_load_content_post() {
		ob_start();
		...
		if ( ! class_exists( 'Elementor\Thim_Ekit_Widget_Tabs_Post' ) ) {
			include_once THIM_DIR . '/thim-elementor-kit/tabs-post/tabs-post.php';
		}
		$list_course = new \Elementor\Thim_Ekit_Widget_Tabs_Post();
		$widget_settings    = new \Thim_EL_Kit\Elementor\Hooks();
		$settings = $widget_settings->get_widget_settings( intval( $params['page_id'] ),
	...
	add_action( 'wp_ajax_thim_load_content_post', 'thim_ajax_load_content_post' );
	add_action( 'wp_ajax_nopriv_thim_load_content_post', 'thim_ajax_load_content_post' );
}

if ( ! function_exists( 'thim_custom_page_title_search_page' ) ) {
	function thim_custom_page_title_search_page( $heading_title ) {
		if ( is_search() && empty( $_GET['s'] ) && $_GET['post_type'] === 'post') {
			$heading_title = esc_html__( 'Trending', 'eduma' );
		}
		return $heading_title;
	}

	add_filter( 'thim-ekit/widgets/page-title', 'thim_custom_page_title_search_page' );
}