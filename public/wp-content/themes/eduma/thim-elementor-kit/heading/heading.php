...
class Thim_Ekit_Widget_Heading extends Widget_Base {
	...
	public function get_icon() {
		return 'eduma-eicon thim-widget-icon thim-widget-icon-heading';
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'description' => esc_html__( '"Main Title" Settings will be worked, If you use this {{something}} format',
					'eduma' ),
			]
		);
		$this->add_control(
			...
			[
				...
				'type'        => Controls_Manager::TEXT,
				'placeholder' => esc_html__( 'Add your text here', 'eduma' ),
				'description' => esc_html__( 'Write the Main title for the heading. It will be at the end of the title.', 'eduma' ),
				'label_block' => true
			]
		);
		...
		$this->add_group_control(
			...
			[
				...
				'label'    => esc_html__( 'Main Title Typography', 'eduma' ),
				'selector' => '{{WRAPPER}} .sc_heading .thim-color',
			]
		);
		$this->end_controls_section();
	...
	public function render_html( $settings ) {
		...
		if ( isset( $settings['title'] ) && $settings['title'] ) {
			...
			echo sprintf( '<%s class="title">%s %s</%s>', $title_tag, wp_kses_post( str_replace( array( '{{', '}}' ), array( '<span class="thim-color">', '</span>' ),$settings['title']) ), $title_main, $title_tag );
		}

		if ( isset( $settings['clone_title'] ) && isset( $settings['title'] ) && $settings['clone_title'] ) {
			echo '<div class="clone">' . wp_kses_post( str_replace( array( '{{', '}}' ), '', $settings['title']) ) . '</div>';
		}
