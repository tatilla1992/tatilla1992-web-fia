...
function thim_eduma_register_meta_boxes_post( $meta_boxes ) {
	...
	$meta_boxes[] = array(
		...
		'title'      => __( 'Post meta', 'eduma' ),
		'post_types' => 'post',
		'fields'     => array(
			array(
				'label'   => __( 'Reading Time', 'eduma' ),
				'id'      => $prefix . 'post_read_time',
				'desc'	  => __( 'Set a custom time to read, listen, or watch.', 'eduma' ),
				'type'    => 'text',
			),
		)
	);
...
if ( ! function_exists( 'thim_add_metabox_settings' ) ) {
	function thim_add_metabox_settings( $meta_box, $prefix ) {
		...
		// check hidden header options when heading build thim elementor kit
		if ( thim_customizer_extral_class( 'header' ) == ' hidden' ) {
			unset( $meta_box['tabs']['header'] );
		}
		// check hidden Title & Layout when page course build thim elementor kit
		if ( isset( $_GET['post'] ) && class_exists( 'LearnPress' ) && ( thim_customizer_extral_class( 'archive-course', array( 'all', 'course_page' ) ) == ' hidden' ) && ( get_option( 'learn_press_courses_page_id' ) == $_GET['post'] ) ) {
			unset( $meta_box['tabs']['title'] );
			unset( $meta_box['tabs']['layout'] );
		}
		// check hidden Title & Layout when page shop build thim elementor kit
		if ( isset( $_GET['post'] ) && class_exists( 'WooCommerce' ) && ( thim_customizer_extral_class( 'archive-product', array( 'all', 'shop_page' ) ) == ' hidden' ) && ( get_option( 'woocommerce_shop_page_id' ) == $_GET['post'] ) ) {
			unset( $meta_box['tabs']['title'] );
			unset( $meta_box['tabs']['layout'] );
		}
