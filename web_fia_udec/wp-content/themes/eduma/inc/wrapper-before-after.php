...
if ( ! function_exists( 'thim_wrapper_layout' ) ) :
	function thim_wrapper_layout() {
		...
		if ( is_page() || is_single() || $get_post_type == "lp_collection" || thim_check_is_shop() || thim_check_courses_page() ) {
			$postid = get_the_ID();
			if ( thim_check_is_shop() ) {
				$postid = get_option( 'woocommerce_shop_page_id' );
			}
			if ( thim_check_courses_page() ) {
				$postid = learn_press_get_page_id( 'courses' );
 			}
 			$wrapper_layout = get_theme_mod( $prefix . '_single_layout', 'sidebar-right' );

			// fix page shop and courses
			if ( thim_check_is_shop() || thim_check_courses_page() ) {
			...
			if ( ( thim_lp_style_single_course() == 'new-1' || thim_lp_style_single_course() == 'layout_style_3' ) && $prefix == 'thim_learnpress' && ! thim_check_courses_page() ) {
				$wrapper_layout = 'full-content';
 			}
