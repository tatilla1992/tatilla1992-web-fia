...
if ( $posts_display->have_posts() ) :
	...
	while ( $posts_display->have_posts() ) : $posts_display->the_post();
		...
		$img   = thim_get_feature_image( get_post_thumbnail_id(), 'full' );
		$class = '';
        ...
        $html .= '<div class="info_post"><div class="inner-info">';
        $html .= '<h5 class="inner-title">'. get_the_title().'</h5>';
...
<?php endif; ?>
	...
	<div class="wrapper-gallery-filter isotope-layout" itemscope itemtype="http://schema.org/ItemList" data-column="<?php echo esc_attr( $columns ) ?>">
		<?php
		echo ent2ncr( $html );
		?>
	</div>
